<x-layouts.base>
    ...
    <div class="w-screen h-screen flex items-center justify-center">
        <form class="flex flex-col space-y-4" method="post" action="{{ route('password.update') }}">
            ...
            <!-- Email address -->
            <x-input name="email" type="email" label="{{ __('Email') }}" icon="o-at-symbol" error-field="email" class="min-w-96" value="{{ $request->email }}" />

            <!-- Password -->
            <x-input name="password" type="password" label="{{ __('Password') }}" icon="o-key" error-field="password" class="min-w-96" />

            <!-- Password confirmation -->
            <x-input name="password_confirmation" type="password" label="{{ __('Password Confirmation') }}" icon="o-key" error-field="password_confirmation" class="min-w-96" />
